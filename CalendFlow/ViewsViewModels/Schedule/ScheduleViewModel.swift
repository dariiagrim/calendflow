...
class ScheduleViewModel: ObservableObject {
    @Published var eventWithIntersections: [EventWithIntersections] = []
    
    init(events: [Event]) {
        eventWithIntersections = getEventsWithIntersections(events: events)
    }
 
    func getEventsWithIntersections(events: [Event]) -> [EventWithIntersections] {
